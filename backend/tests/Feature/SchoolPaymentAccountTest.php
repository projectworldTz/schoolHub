 ...
 * see on their dashboard as where to send fee payments. Reading the list is
 * open to any authenticated school user (parents included); writing it is
 * gated behind school-settings.manage.
 */
class SchoolPaymentAccountTest extends TestCase
{
    ...
    public function test_a_school_owner_can_update_and_delete_a_payment_account(): void
    {
        ...
        $update = $this->actingAs($owner, 'web')->putJson("/api/school/payment-accounts/{$created['id']}", [
            'account_name' => 'CRDB Bank (Main Branch)',
            'account_number' => '0150234567',
            'currency' => 'TZS',
        ]);
        $update->assertOk();
        $update->assertJsonPath('data.account_name', 'CRDB Bank (Main Branch)');
