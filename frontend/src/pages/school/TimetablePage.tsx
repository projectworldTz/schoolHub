    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent>
        ...
        <div className="grid gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><label className="text-sm">Periods per week<Input type="number" min={1} max={20} value={periods} onChange={(e) => setPeriods(Number(e.target.value))} /></label><label className="text-sm">Double blocks<Input type="number" min={0} max={Math.floor(periods / 2)} value={doubles} onChange={(e) => setDoubles(Number(e.target.value))} /></label></div>
        </div>
